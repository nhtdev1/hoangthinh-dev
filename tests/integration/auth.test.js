...
describe('Auth routes', () => {
	...
	describe('POST /v1/auth/logout', () => {
		...
		test('should return 404 error is refresh token is not found in the database', async () => {
			await insertUsers([userOne]);
			const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
			const refreshToken = tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
	...
	describe('POST /v1/auth/refresh-tokens', () => {
		test('should return 200 and new auth tokens if refresh token is valid', async () => {
			await insertUsers([userOne]);
			const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
			const refreshToken = await tokenService.generateToken(userOne._id, expires, tokenTypes.REFRESH);
			await tokenService.saveToken(refreshToken, userOne._id, expires, tokenTypes.REFRESH);
		...
		test('should return 401 error if refreh token is expired', async () => {
			await insertUsers([userOne]);
	...
	describe('POST /v1/auth/reset-password', () => {
		...
		test('should return 401 if reset password is blacklisted', async () => {
			await insertUsers([userOne]);
			const expires = moment().add(config.jwt.resetPasswordExpirationMinutes, 'minutes');
			const resetPasswordToken = tokenService.generateToken(userOne._id, expires, tokenTypes.RESET_PASSWORD);
	...
	describe('POST /v1/auth/send-verification-email', () => {
		...
		test('should return 204 and verification email to the user', async () => {
			await insertUsers([userOne]);
	...
	describe('POST /v1/auth/verify-email', () => {
		...
		test('should return 401 if verify email is expired', async () => {
			await insertUsers([userOne]);
			const expires = moment().subtract(1, 'minutes');
			const verifyEmailToken = tokenService.generateToken(userOne._id, expires);
			await tokenService.saveToken(verifyEmailToken, userOne._id, expires, tokenTypes.VERIFY_EMAIL);
...
describe('Auth middleware', () => {
	...
	test('should call next with unauthorized error if access token is not an access token', async () => {
		await insertUsers([userOne]);
