...
describe('User routes', () => {
	describe('POST /v1/users', () => {
		...
		test('should return 400 error if email is invalid', async () => {
			...
			await request(app).post('/v1/users').set('Authorization', `Bearer ${adminAccessToken}`).expect(httpStatus.BAD_REQUEST);
		});
	...
	describe('GET /v1/users', () => {
		...
		test('should correctly sort the returned array if multiple sorting crieria are specified', async () => {
			await insertUsers([userOne, userTwo, admin]);
	...
	describe('DELETE /v1/users/:userId', () => {
		...
		test('should return 204 error if admin is trying to delete another user', async () => {
			await insertUsers([userOne, admin]);
			await request(app)
				.delete(`/v1/users/${userOne._id}`)
				.set('Authorization', `Bearer ${adminAccessToken}`)
	...
	describe('PATH /v1/users/:userId', () => {
		...
		test('shoild return 403 if user is updating another user', async () => {
			await insertUsers([userOne, userTwo]);
			const updateBody = { name: faker.name.findName() };
		...
		test('should return 400 if email is my email', async () => {
			await insertUsers([userOne]);
